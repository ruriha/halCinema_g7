...
import com.example.halCinema.model.TimeTable4;

@Repository
public interface TimeTableRepository4 extends JpaRepository<TimeTable4, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("update timeTable4 tt set tt.freeStatus = true where tt.time.timeId = ?1 and tt.screen.screenId = ?2")
    ...
    @Modifying
    @Transactional
    @Query("update timeTable4 tt set tt.freeStatus = false where tt.time.timeId = ?1 and tt.screen.screenId = ?2")

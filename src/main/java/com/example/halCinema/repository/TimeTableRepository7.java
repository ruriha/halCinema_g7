...
import com.example.halCinema.model.TimeTable7;

@Repository
public interface TimeTableRepository7 extends JpaRepository<TimeTable7, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("update timeTable7 tt set tt.freeStatus = true where tt.time.timeId = ?1 and tt.screen.screenId = ?2")
    ...
    @Modifying
    @Transactional
    @Query("update timeTable7 tt set tt.freeStatus = false where tt.time.timeId = ?1 and tt.screen.screenId = ?2")

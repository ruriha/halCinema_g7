...
import com.example.halCinema.model.TimeTable6;

@Repository
public interface TimeTableRepository6 extends JpaRepository<TimeTable6, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("update timeTable6 tt set tt.freeStatus = true where tt.time.timeId = ?1 and tt.screen.screenId = ?2")
    ...
    @Modifying
    @Transactional
    @Query("update timeTable6 tt set tt.freeStatus = false where tt.time.timeId = ?1 and tt.screen.screenId = ?2")

...
import com.example.halCinema.model.TimeTable5;

@Repository
public interface TimeTableRepository5 extends JpaRepository<TimeTable5, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("update timeTable5 tt set tt.freeStatus = true where tt.time.timeId = ?1 and tt.screen.screenId = ?2")
    ...
    @Modifying
    @Transactional
    @Query("update timeTable5 tt set tt.freeStatus = false where tt.time.timeId = ?1 and tt.screen.screenId = ?2")
